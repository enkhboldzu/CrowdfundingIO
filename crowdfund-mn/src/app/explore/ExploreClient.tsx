...
function ExploreContent() {
  ...
  const [stats,    setStats]    = useState({ total: 0, raised: 0, backers: 0 });

  /* Sync search input when navigating from Navbar (?q=) */
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { setQuery(urlQ); }, [urlQ]);

  /* Fetch from API whenever category or sort changes */
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLoading(true);
    ...
    fetch(`/public-api/projects?${qs}`)
      ...
      .then(data => {
        ...
        setStats({
          total:   data.total        ?? 0,
          raised:  data.totalRaised  ?? 0,
          backers: data.totalBackers ?? 0,
        });
      })
      .catch(() => setProjects([]))
  ...
  function handleCategoryChange(value: string) {
    ...
    if (value === "all") qs.delete("category");
    else qs.set("category", value);
    ...
    <>
      <main>
        ...
        <section className="gradient-brand-hero pt-28 pb-16 relative overflow-hidden">
          <div aria-hidden className="absolute -top-24 -right-24 w-96 h-96 rounded-full opacity-10"
            style={{ background: "radial-gradient(circle, #93C5FD, transparent 70%)" }} />
          <div aria-hidden className="absolute -bottom-32 -left-20 w-80 h-80 rounded-full opacity-10"
          ...
          <div className="container-page relative z-10">
            <div className="max-w-2xl">
              ...
              <div className="relative mb-8">
                ...
                />
                {query && (
                  <button onClick={handleClearQuery} aria-label="Хайлт цэвэрлэх"
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
                <div className="flex items-center gap-2 text-white/70">
                  ...
                  <span>
                    <strong className="text-white font-bold">{stats.total}</strong> идэвхтэй төсөл
                  </span>
                </div>
          ...
          <div className="container-page">
            <div className="flex items-center gap-3 py-3 sm:py-3.5">
              ...
              <span className="hidden sm:block flex-shrink-0 text-slate-400 text-sm whitespace-nowrap">
                {loading ? "..." : `${filtered.length} үр дүн`}
              </span>
        ...
        <section className="bg-slate-50 min-h-[60vh]">
          <div className="container-page py-10">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-7">
                ...
                {category !== "all" && (
                  <span className="inline-flex items-center gap-1.5 bg-blue-100 text-blue-800 text-xs font-semibold px-3 py-1.5 rounded-full">
                    ...
                    <button onClick={() => handleCategoryChange("all")} aria-label="Ангилал арилгах" className="ml-0.5 hover:text-blue-600">
                ...
                {query.trim() && (
                  <span className="inline-flex items-center gap-1.5 bg-blue-100 text-blue-800 text-xs font-semibold px-3 py-1.5 rounded-full">
                    &ldquo;{query}&rdquo;
                    <button onClick={handleClearQuery} aria-label="Хайлт арилгах" className="ml-0.5 hover:text-blue-600">
                      <svg className="w-3 h-3" viewBox="0 0 20 20" fill="currentColor"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg>
                    </button>
                  </span>
                )}
            ...
            <p className="text-slate-400 text-sm mb-6 sm:hidden">
              {loading ? "Уншиж байна..." : `${filtered.length} үр дүн`}
            </p>
