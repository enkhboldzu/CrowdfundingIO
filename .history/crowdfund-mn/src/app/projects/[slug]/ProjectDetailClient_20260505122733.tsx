...
import { MOCK_REWARD_TIERS, MOCK_UPDATES } from "@/lib/mock-data";
...
export function ProjectDetailClient({ project }: Props) {
  ...
  const tiers   = MOCK_REWARD_TIERS[project.slug] ?? [];
  const updates = MOCK_UPDATES[project.slug] ?? [];
      ...
      <main className="min-h-screen bg-slate-50">
        ...
        <div className="bg-white border-b border-slate-100">
          <div className="container-page py-3">
            <Link
              href="/explore"
              className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-800 transition-colors font-medium"
            >
...
function FundingCard({ project, percent }: { project: Project; percent: number }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-card p-6">
      ...
      {/* CTA */}
      <button className="w-full bg-blue-800 hover:bg-blue-900 active:bg-blue-950 text-white font-bold text-base py-3.5 rounded-xl transition-colors shadow-cta flex items-center justify-center gap-2">
        Одоо дэмжих
        <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
